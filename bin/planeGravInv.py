...
class ACGravity(object):
    def __init__(self, domain, w_e, gz_e, rho_0, mu, m0,
         atol=1.0, rtol=1.0, iter_max=100, pde_tol=1e-8, output_name='solutions', verbose=False):
    ...
    def solve(self):
        ...
        m = PCG(r, self.Aprod, self.m0, self.Msolve, self.bilinearform,
                 ...
                 initial_guess=True, verbose=self.verbose)
...
if config.VerboseLevel == "low":
    grav = ACGravity(dom, w_e, gz_e, rho_e, mu, m0, atol, rtol, iter_max, pdetol, config.output_name, False)
    m, gz =grav.solve()
elif config.VerboseLevel == "medium":
    grav = ACGravity(dom, w_e, gz_e, rho_e, mu, m0, atol, rtol, iter_max, pdetol, config.output_name, True)
    m, gz =grav.solve()
elif config.VerboseLevel == "high":
    grav = ACGravity(dom, w_e, gz_e, rho_e, mu, m0, atol, rtol, iter_max, pdetol, config.output_name)
    m, gz =grav.solve2()
